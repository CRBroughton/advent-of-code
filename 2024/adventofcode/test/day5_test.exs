defmodule Day5Test do
  ...
  test "splits the rules and updates into separate strings" do
    input = """
    47|53
    97|13
    97|61
    ...
    assert Day5.splitRulesAndUpdates(input) ===
             {[
                "47|53",
                "97|13",
                "97|61",
  ...
  test "Creates a Map of the rules" do
    input = [
      "47|53",
      "97|13",
      "97|61",
    ...
    assert Day5.createRulesMapSet(input) ===
             MapSet.new([
               {29, 13},
               {47, 13},
               {47, 29},
  ...
  test "Creates a updates array" do
    input = [
      "75,47,61,53,29",
      "97,61,53,29,13",
      "75,29,13",
    ...
    assert Day5.createUpdatesArray(input) === [
             [75, 47, 61, 53, 29],
             [97, 61, 53, 29, 13],
             [75, 29, 13],
             [75, 97, 47, 61, 53],
  ...
  test "gets the middle number of an update" do
    assert Day5.getMiddleNumber([75, 47, 61, 53, 29]) === 61
  end

  test "tests validity of a given update" do
    input = """
    47|53
    97|13
    97|61
    ...
    {rules, updates} = Day5.splitRulesAndUpdates(input)
    rules = Day5.createRulesMapSet(rules)
    updates = Day5.createUpdatesArray(updates)

    firstUpdate = Enum.at(updates, 0)
    assert Day5.isValidOrder?(rules, firstUpdate) === true

    fourthUpdateShouldFail = Enum.at(updates, 3)
    assert Day5.isValidOrder?(rules, fourthUpdateShouldFail) === false
  end
